# Generated by Selenium IDE
import pytest
import time
import json
from selenium import webdriver
from selenium.webdriver.common.by import By
from selenium.webdriver.common.action_chains import ActionChains
from selenium.webdriver.support import expected_conditions
from selenium.webdriver.support.wait import WebDriverWait
from selenium.webdriver.common.keys import Keys
from selenium.webdriver.common.desired_capabilities import DesiredCapabilities

class TestCasodeprueba299():
  def setup_method(self, method):
    self.driver = webdriver.Chrome()
    self.vars = {}
  
  def teardown_method(self, method):
    self.driver.quit()
  
  def test_casodeprueba299(self):
    self.driver.get("http://localhost:1111/admin/user/new")
    self.driver.set_window_size(1552, 832)
    self.driver.find_element(By.ID, "usersName").click()
    self.driver.find_element(By.ID, "usersName").send_keys("Juan Silva3")
    self.driver.find_element(By.ID, "userEmail").click()
    self.driver.find_element(By.ID, "userEmail").send_keys("juanes.silva@correounivalle.edu.com")
    self.driver.find_element(By.ID, "userPassword").click()
    self.driver.find_element(By.ID, "userPassword").send_keys("Juansilva123!")
    self.driver.find_element(By.CSS_SELECTOR, "#userNewForm > .form-group:nth-child(4) > .form-control").click()
    self.driver.find_element(By.CSS_SELECTOR, ".has-error > .form-control").send_keys("Juansilva123!		")
    self.driver.find_element(By.CSS_SELECTOR, ".row").click()
    self.driver.find_element(By.CSS_SELECTOR, ".row").click()
    self.driver.find_element(By.CSS_SELECTOR, "#userNewForm > .form-group:nth-child(4) > .form-control").send_keys("Juansilva123!")
    self.driver.find_element(By.ID, "container").click()
    self.driver.find_element(By.ID, "btnUserAdd").click()
    assert self.driver.find_element(By.ID, "notify_message").text == "User account inserted"
  
